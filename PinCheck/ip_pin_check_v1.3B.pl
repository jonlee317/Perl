...
print("Enter Top Cell Name (e.g. X001T001):\t");
chomp($design = <STDIN>);
print("Enter LIB type, typical or typ, slow or wc, fast or bc:\t");
chomp($libtype = <STDIN>);
print("Reference File Contains List of IO's of the ip. e.g. (D0_HSTXEN INPUT)   \n");
print("You can use one of the IP views as your reference e.g. LEF or LIB or RTL or reference file name)\n");
print("Enter Reference File:\t");
chomp($ref_file = <STDIN>);
print("Views to Compare: ALL | LEF | LIB | SPI | RTL \n");
print("Enter views:\t");
chomp($views = <STDIN>);
print("\n\n");
...
die "Error opening file! ${design}.LEF";
...
{
open (lib_org, "< $libdir\\${design}_${libtype}.lib") ||
die "Error opening file! ${design}_${libtype}.lib";
...
die "Error opening file! ${design}.spi";
...
die "Error opening file! ${design}.v";
...
foreach $rtl (@rtl)
{
       ...
       elsif (($rtl =~ /\b^input\b/) && ($end_pin_list == 0))
       {
	         $rtl=~ s/\s+//g;
	         $rtl=~ s/input//g;
	         @hold3=split(/\;/,$rtl);
       ...
       elsif (($rtl =~ /\b^output\b/) && ($end_pin_list == 0))
       {
	   $rtl=~ s/\s+//g;
	   $rtl=~ s/output//g;
	   @hold3=split(/\;/,$rtl);
       ...
       elsif (($rtl =~ /\b^inout\b/) && ($end_pin_list == 0))
       {
	   $rtl=~ s/\s+//g;
	   $rtl=~ s/inout//g;
	   @hold3=split(/\;/,$rtl);
       ...
       elsif (($rtl =~ /\bendmodule\b/) && ($end_pin_list == 0))
       {
            $end_pin_list = 1;
	    $rtl="";
       }
...
#
# 1)  under the Verilog Model section
# 
# 1a)  $rtl =~ /\binput\b/  has been changed to:  rtl =~ /\b^input\b/
# 1b)  $rtl =~ /\boutput\b/  has been changed to:  rtl =~ /\b^output\b/
# 1c)  $rtl =~ /\binout\b/  has been changed to:  rtl =~ /\b^inout\b/
#
# Summary:  This change was made to prevent the detection of INPUT/OUTPUT/INOUT used in comments
#